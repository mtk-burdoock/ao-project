...
@export var _main_camera:Camera2D
@export var _fpsLabel:Label
@export var _map_container:Node2D
...
func _ready():
	...
	ui.initialize(_player_data, _protocol, _main_panel, _rich_text_label)
...
func _on_disconnected():	
	if Configuration.interface_mode == 1:		
		scene = load("res://scenes/LobbySceneDesktop.tscn").instantiate()
	else:
		scene = load("res://scenes/LobbySceneMobile.tscn").instantiate()
