...
@onready var select_class = find_child("SelectClase") 
@onready var select_gender = find_child("SelectGenero") 
@onready var select_home = find_child("SelectHogar")  
@onready var select_race = find_child("SelectRaza") 
 
func initialize(protocol:GameProtocol) -> void:
	_protocol = protocol 

func _ready() -> void:
	select_class.connect("pressed_next", Callable(self, "_on_select_class").bind(1))
	select_class.connect("pressed_previus", Callable(self, "_on_select_class").bind(-1))
	
	select_gender.connect("pressed_next", Callable(self, "_on_select_gender").bind(1))
	select_gender.connect("pressed_previus", Callable(self, "_on_select_gender").bind(-1))
	
	select_home.connect("pressed_next", Callable(self, "_on_select_home").bind(1))
	select_home.connect("pressed_previus", Callable(self, "_on_select_home").bind(-1))
	
	select_race.connect("pressed_next", Callable(self, "_on_select_race").bind(1))
	select_race.connect("pressed_previus", Callable(self, "_on_select_race").bind(-1))

	_protocol.connect("parse_data", Callable(self, "_on_parse_data"))
...
func _on_BtnSubmit_pressed() -> void:
	var user_name = find_child("UserName").text
	var user_password = find_child("UserPassword").text
	var user_email = find_child("UserEmail").text
