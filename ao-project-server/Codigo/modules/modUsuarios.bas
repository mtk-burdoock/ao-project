...
Public Sub CheckUserLevel(ByVal Userindex As Integer, Optional ByVal PrintInConsole As Boolean = True)
    ...
    With UserList(Userindex)
        Do While .Stats.Exp >= .Stats.ELU
            ...
            .Stats.MinHp = .Stats.MaxHp
            Call modParty.ActualizarSumaNivelesElevados(Userindex)
...
Public Sub UserDie(ByVal Userindex As Integer, Optional ByVal AttackerIndex As Integer = 0)
    ...
    With UserList(Userindex)
        ...
        If .PartyIndex > 0 Then
            Call modParty.ObtenerExito(Userindex, .Stats.ELV * -10 * modParty.CantMiembros(Userindex), .Pos.Map, .Pos.X, .Pos.Y)
        End If
        ...
        If AttackerIndex <> 0 Then
            If .flags.SlotReto > 0 Then
                Call modRetos.UserDieFight(Userindex, AttackerIndex, False)
            End If
        End If
    End With
    Exit Sub
...
Sub Tilelibre(ByRef Pos As tWorldPos, ByRef nPos As tWorldPos, ByRef obj As obj, ByRef PuedeAgua As Boolean, ByRef PuedeTierra As Boolean)
    On Error GoTo ErrorHandler
