...
Public Function InVisionRangeAndMap(ByVal Userindex As Integer, ByRef OtherUserPos As tWorldPos) As Boolean
...
Private Function RhombLegalPos(ByRef Pos As tWorldPos, ByRef vX As Long, ByRef vY As Long, ByVal Distance As Long, Optional PuedeAgua As Boolean = False, _
...
Public Function RhombLegalTilePos(ByRef Pos As tWorldPos, ByRef vX As Long, ByRef vY As Long, ByVal Distance As Long, ByVal ObjIndex As Integer, ByVal ObjAmount As Long, ByVal PuedeAgua As Boolean, ByVal PuedeTierra As Boolean) As Boolean
    On Error GoTo ErrorHandler
    Dim i           As Long
...
Sub ClosestLegalPos(Pos As tWorldPos, ByRef nPos As tWorldPos, Optional PuedeAgua As Boolean = False, Optional PuedeTierra As Boolean = True, Optional ByVal CheckExitTile As Boolean = False)
...
Public Sub ClosestStablePos(Pos As tWorldPos, ByRef nPos As tWorldPos)
...
Sub HeadtoPos(ByVal Head As eHeading, ByRef Pos As tWorldPos)
    Select Case Head
        Case eHeading.NORTH
            Pos.Y = Pos.Y - 1
...
Function FindDirection(Pos As tWorldPos, Target As tWorldPos) As eHeading
    Dim X As Integer
    Dim Y As Integer
    X = Pos.X - Target.X
    Y = Pos.Y - Target.Y
