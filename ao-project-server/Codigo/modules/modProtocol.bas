...
Public Function HandleIncomingData(ByVal Userindex As Integer) As Boolean
    ...
    Select Case packetID
        ...
        Case ClientPacketID.Quest
            Call modQuests.HandleQuest(Userindex)
            
        Case ClientPacketID.QuestAccept
            Call modQuests.HandleQuestAccept(Userindex)
        
        Case ClientPacketID.QuestListRequest
            Call modQuests.HandleQuestListRequest(Userindex)
        
        Case ClientPacketID.QuestDetailsRequest
            Call modQuests.HandleQuestDetailsRequest(Userindex)
        
        Case ClientPacketID.QuestAbandon
            Call modQuests.HandleQuestAbandon(Userindex)
        ...
        Case ClientPacketID.AddAmigos
            Call modAmigos.HandleAddAmigo(Userindex)

        Case ClientPacketID.DelAmigos
            Call modAmigos.HandleDelAmigo(Userindex)

        Case ClientPacketID.OnAmigos
            Call modAmigos.HandleOnAmigo(Userindex)

        Case ClientPacketID.MsgAmigos
            Call modAmigos.HandleMsgAmigo(Userindex)
...
Private Sub HandleGMCommands(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        Select Case Command
            ...
            Case modDeclaraciones.eGMCommands.DoBackUp
...
Private Sub HandlePartyLeave(ByVal Userindex As Integer)
    Call UserList(Userindex).incomingData.ReadByte
    Call modParty.SalirDeParty(Userindex)
End Sub

Private Sub HandlePartyCreate(ByVal Userindex As Integer)
    Call UserList(Userindex).incomingData.ReadByte
    If Not modParty.PuedeCrearParty(Userindex) Then Exit Sub
    Call modParty.CrearParty(Userindex)
End Sub

Private Sub HandlePartyJoin(ByVal Userindex As Integer)
    Call UserList(Userindex).incomingData.ReadByte
    Call modParty.SolicitarIngresoAParty(Userindex)
End Sub
...
Private Sub HandlePartyMessage(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        If LenB(Chat) <> 0 Then
            Call modStatistics.ParseChat(Chat)
            Call modParty.BroadCastParty(Userindex, Chat)
        End If
        Call .incomingData.CopyBuffer(buffer)
    End With
ErrorHandler:
...
Private Sub HandlePartyOnline(ByVal Userindex As Integer)
    Call UserList(Userindex).incomingData.ReadByte
    Call modParty.OnlineParty(Userindex)
End Sub
...
Private Sub HandleChangePassword(ByVal Userindex As Integer)
    Dim oSHA256 As clsSHA256
    Set oSHA256 = New clsSHA256
    If UserList(Userindex).incomingData.Length < 5 Then
        Err.Raise UserList(Userindex).incomingData.NotEnoughDataErrCode
        Exit Sub
    End If
...
Private Sub HandlePartyKick(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        If UserPuedeEjecutarComandos(Userindex) Then
            tUser = NameIndex(UserName)
            If tUser > 0 Then
                Call modParty.ExpulsarDeParty(Userindex, tUser)
            Else
        ...
        If UserPuedeEjecutarComandos(Userindex) Then
            ...
            If tUser > 0 Then
                If (UserDarPrivilegioLevel(UserName) And rank) <= (.flags.Privilegios And rank) Then
                    Call modParty.TransformarEnLider(Userindex, tUser)
                Else
...
Private Sub HandlePartyAcceptMember(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        Else
            bUserVivo = True
        End If
        If modParty.UserPuedeEjecutarComandos(Userindex) And bUserVivo Then
            tUser = NameIndex(UserName)
            If tUser > 0 Then
                If (UserList(tUser).flags.Privilegios And rank) <= (.flags.Privilegios And rank) Then
                    Call modParty.AprobarIngresoAParty(Userindex, tUser)
                Else
...
Public Sub HandleMoveItem(ByVal Userindex As Integer)
    With UserList(Userindex)
        ...
        Call .incomingData.ReadByte
        Call modInvUsuario.moveItem(Userindex, originalSlot, newSlot)
    End With
End Sub
...
Private Sub HandleEnviaCvc(ByVal Userindex As Integer)
    With UserList(Userindex)
        ...
        Call modClanvsClan.Enviar(Userindex, .flags.TargetUser)
    End With
End Sub
...
Private Sub HandleAceptarCvc(ByVal Userindex As Integer)
    With UserList(Userindex)
        .incomingData.ReadByte
        If .flags.TargetUser = 0 Then Exit Sub
        Call modClanvsClan.Aceptar(Userindex, .flags.TargetUser)
    End With
End Sub
...
Private Sub HandleIrCvc(ByVal Userindex As Integer)
    With UserList(Userindex)
        .incomingData.ReadByte
        Call modClanvsClan.ConectarCVC(Userindex, True)
    End With
End Sub
...
On Error GoTo ErrorHandler
    With UserList(Userindex)
        ...
        If Len(ListUsers) >= 1 Then
            ...
            Call modRetos.SendFight(Userindex, GldRequired, Users)
        End If
        Call .incomingData.CopyBuffer(buffer)
    End With
ErrorHandler:
...
On Error GoTo ErrorHandler
    With UserList(Userindex)
        ...
        If Len(UserName) >= 1 Then
            Call modRetos.AcceptFight(Userindex, UserName)
        End If
        Call .incomingData.CopyBuffer(buffer)
    End With
ErrorHandler:
