Attribute VB_Name = "modClanvsClan"
Option Explicit
...
Public Sub Enviar(ByVal Userindex As Integer, ByVal targetIndex As Integer)
    With UserList(Userindex)
        ...
        Call modProtocol.WriteConsoleMsg(targetIndex, "El clan " & modGuilds.GuildName(my_Guild) & " desafia tu clan a un duelo de modalidad Clan vs Clan, si aceptas hazle click y tipea /ACVC.", FontTypeNames.FONTTYPE_GUILD)
        Call modProtocol.WriteConsoleMsg(targetIndex, "La cantidad maxima de usuarios por clan es de : " & CStr(menorCant) & ".", FontTypeNames.FONTTYPE_GUILD)
        Call modProtocol.WriteConsoleMsg(Userindex, "Ahora debes esperar que el lider acepte.", FontTypeNames.FONTTYPE_GUILD)
    End With
End Sub
...
Public Sub Aceptar(ByVal Userindex As Integer, ByVal targetIndex As Integer)
    With UserList(Userindex)
        ...
        Else
            Call modProtocol.WriteConsoleMsg(Userindex, UserList(targetIndex).Name & " no solicito ningun Clan vs Clan.", FontTypeNames.FONTTYPE_GUILD)
        End If
    End With
End Sub
...
Private Sub Iniciar(ByVal userSend As Integer, ByVal userAccept As Integer, ByVal Guild_Desafiado As Integer, ByVal Guild_Desafiante As Integer, ByVal max_Users As Byte)
    With CVC_Info
        ...
        ReDim .Guild(1).modUsuarios(1 To max_Users) As Integer
        ReDim .Guild(2).modUsuarios(1 To max_Users) As Integer
        .max_Users = max_Users
        Dim j As Long
        For j = 1 To max_Users
            .Guild(1).modUsuarios(j) = -1
            .Guild(2).modUsuarios(j) = -1
        Next j
        For j = 1 To 2
            Call SendData(SendTarget.ToGuildMembers, .Guild(j).Guild_Index, modProtocol.PrepareMessageConsoleMsg("CLAN VS CLAN > " & modGuilds.GuildName(.Guild(1).Guild_Index) & " vs " & modGuilds.GuildName(.Guild(2).Guild_Index) & " cada clan con " & CStr(.max_Users) & " Participantes, para participar tipea /IRCVC.", FontTypeNames.FONTTYPE_GUILD))
        Next j
...
Private Sub EnviarMensajeCVC(ByVal to_Guild As Byte, ByRef send_Msg As String)
    With CVC_Info
        ...
        If (to_Guild = 0) Then
            For j = 1 To 2
                With .Guild(j)
                    For i = 1 To UBound(.modUsuarios())
                        If .modUsuarios(i) <> -1 Then
                            If UserList(.modUsuarios(i)).ConnID <> -1 Then
                                Call modProtocol.WriteConsoleMsg(.modUsuarios(i), send_Msg, FontTypeNames.FONTTYPE_GUILD)
                            End If
                        End If
                    Next i
                End With
            ...
            Exit Sub
        End If
        For j = 1 To UBound(.Guild(to_Guild).modUsuarios())
            With .Guild(to_Guild)
                For i = 1 To UBound(.modUsuarios())
                    If .modUsuarios(i) <> -1 Then
                        If UserList(.modUsuarios(i)).ConnID <> -1 Then
...
Private Sub ReiniciarCVC()
    ...
    With CVC_Info
        For i = 1 To 2
            With .Guild(i)
                For j = 1 To UBound(.modUsuarios())
                    If (.modUsuarios(j) <> -1) Then
                        If (UserList(.modUsuarios(j)).ConnID <> -1) Then
                            Call Get_Pos_By_Guild(.modUsuarios(j), CByte(i), sX, sY)
                            If (sX <> 0) And (sY <> 0) Then
                                Call WarpUserChar(.modUsuarios(j), MAPA_CVC, sX, sY, True)
                            End If
                        End If
                    End If
                Next j
...
Private Sub EraseCVC()
    With CVC_Info
        ...
        Dim i As Long
        For j = 1 To 2
            For i = 1 To UBound(.Guild(j).modUsuarios())
                .Guild(j).modUsuarios(i) = -1
            Next i
            .Guild(j).Num_Users = 0
        ...
        .count_Down = 0
        .max_Users = 0
        Erase .Guild(1).modUsuarios
        Erase .Guild(2).modUsuarios
    End With
End Sub
...
Private Sub GanaCVC(ByVal guildWinner As Byte, ByVal guildLooser As Byte)
    With CVC_Info
        ...
        For i = 1 To 2
            For j = 1 To UBound(.Guild(i).modUsuarios())
                With .Guild(i)
                    If (.modUsuarios(j) <> -1) Then
                        Call FindLegalPos(.modUsuarios(j), Ullathorpe.Map, CInt(startX), CInt(startY))
                        If (startX <> 0) And (startY <> 0) Then
                            Call WarpUserChar(.modUsuarios(j), Ullathorpe.Map, startX, startY, True)
                        End If
                        UserList(.modUsuarios(j)).cvcUser.cvc_MaxUsers = 0
                        UserList(.modUsuarios(j)).cvcUser.cvc_Target = 0
                        UserList(.modUsuarios(j)).cvcUser.en_CVC = False
                    End If
                End With
            Next j
        Next i
        ...
        Call SendData(SendTarget.ToAll, 0, modProtocol.PrepareMessageConsoleMsg(sMessage, FontTypeNames.FONTTYPE_GUILD))
        Call EraseCVC
    End With
End Sub
...
Public Sub ConectarCVC(ByVal Userindex As Integer, Optional ByVal check_Enabled As Boolean = False)
    ...
    If (check_Enabled = True) Then
        ...
        If (Can_Ingress(Userindex, ref_Error) = False) Then
            Call modProtocol.WriteConsoleMsg(Userindex, ref_Error, FontTypeNames.FONTTYPE_GUILD)
            Exit Sub
        End If
        guild_Num = Find_Guild_Num(UserList(Userindex).GuildIndex)
    End If
    With CVC_Info.Guild(guild_Num)
        ...
        If (user_Gindex <> 0) Then
            .modUsuarios(user_Gindex) = Userindex
        ...
        Else
            Call modProtocol.WriteConsoleMsg(Userindex, "No puedes entrar al CVC porque tu clan ya tiene " & CStr(CVC_Info.max_Users) & " jugadores.", FontTypeNames.FONTTYPE_GUILD)
        End If
    End With
End Sub
...
Private Function Get_Num_Dies(ByVal guild_Num As Byte) As Byte
    With CVC_Info
        Dim j As Long
        For j = 1 To UBound(.Guild(guild_Num).modUsuarios())
            With .Guild(guild_Num)
                If (.modUsuarios(j) <> -1) Then
                    If (UserList(.modUsuarios(j)).ConnID = -1) Or (UserList(.modUsuarios(j)).flags.Muerto <> 0) Then
