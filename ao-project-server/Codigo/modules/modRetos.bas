...
Public typeRetos(1 To MAX_RETOS_SIMULTANEOS) As tRetos
...
Private Function FreeSlotArena() As Byte
    ...
    For LoopC = 1 To MAX_RETOS_SIMULTANEOS
        If typeRetos(LoopC).Run = False Then
            FreeSlotArena = LoopC
            Exit Function
        End If
    Next LoopC
...
Private Sub RewardUsers(ByVal SlotReto As Byte, ByVal Userindex As Integer)
    ...
    Dim obj As obj
    With UserList(Userindex)
        .Stats.Gld = .Stats.Gld + (typeRetos(SlotReto).RequiredGld * 2)
...
Private Sub GoFight(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        If ValidateFight(Userindex, .RetoTemp.RequiredGld, .RetoTemp.Users) Then
            typeRetos(SlotArena).RequiredGld = .RetoTemp.RequiredGld
            typeRetos(SlotArena).Run = True
            Call PasateInteger(SlotArena, .RetoTemp.Users)
            Call SetUserEvent(SlotArena, typeRetos(SlotArena).Users)
            Call WarpFight(typeRetos(SlotArena).Users)
        End If
    End With
    Exit Sub
ErrorHandler:
...
Public Sub UserDieFight(ByVal Userindex As Integer, ByVal AttackerIndex As Integer, ByVal Forzado As Boolean)
    ...
    If AttackerIndex = 0 Then
        AttackerIndex = AttackerFight(SlotReto, typeRetos(SlotReto).Users(UserList(Userindex).flags.SlotRetoUser).Team)
        Deslogged = True
    End If
    TeamUser = typeRetos(SlotReto).Users(UserList(AttackerIndex).flags.SlotRetoUser).Team
